...
cat /etc/ssh/sshd_config | grep PermitRootLogin >> ${PCI_AUDIT_OUTPUT_DIR}/${HOSTNAME}_Root_Login_Disabled.txt
cat /etc/sysctl.conf >> ${PCI_AUDIT_OUTPUT_DIR}/${HOSTNAME}_Kernel_Parameters.txt
sudo find / -perm -4000 >> ${PCI_AUDIT_OUTPUT_DIR}/${HOSTNAME}_SUID_Binaries.txt 2>&1 || true
sudo find / -perm -2000  >> ${PCI_AUDIT_OUTPUT_DIR}/${HOSTNAME}_SGID_Binaries.txt 2>&1 || true
sudo find / \( -nouser -o -nogroup \) -print >> ${PCI_AUDIT_OUTPUT_DIR}/${HOSTNAME}_No_Owner_Files.txt 2>&1 || true
